...
template <typename AudioEnum>
void AudioComponent<AudioEnum>::loadSound(AudioEnum state,
        ...
        if (!jsonFile.is_open()) {
                ERROR_MESSAGE("Impossible d'ouvrir les sons " + jsonPath +
                              " | state " +
        ...
        try {
                ...
                if (!sound) {
                        ERROR_MESSAGE("Chargement " + jsonPath +
                                      " | fichier son state " +
                                      std::to_string(static_cast<int>(state)) +
                                      "/" + soundPath + " non trouvé");
                        return;
                }
                ...
                ERROR_MESSAGE("Recupération audio " + jsonPath + " | " +
                              e.what());
                return;
        }
...
void AudioComponent<AudioEnum>::play(AudioEnum state) {
        if (sounds[state] != nullptr) {
                ...
                ERROR_MESSAGE("Pas de sons pour l'état " +
                              std::to_string(static_cast<int>(state)));
        }
}

Sprite *IAudioComponent::getOwner() const { return owner; }

void IAudioComponent::setOwner(Sprite *owner) { this->owner = owner; }

template <typename AudioEnum>
AudioComponent<AudioEnum>::AudioComponent(const AudioComponent &other)
    : IAudioComponent(other), sounds(other.sounds) {}

template <typename AudioEnum>
AudioComponent<AudioEnum>::AudioComponent(AudioComponent &&other) noexcept
    : IAudioComponent(std::move(other)), sounds(std::move(other.sounds)) {}

template <typename AudioEnum>
AudioComponent<AudioEnum> &
