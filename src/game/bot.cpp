...
std::ostream &operator<<(std::ostream &os, Bot::State state) {
        switch (state) {
        case Bot::State::patrol:
                os << "patrol";
                break;
        case Bot::State::seek:
                os << "seek";
                break;
        case Bot::State::engage:
                os << "engage";
                break;
        default:
                os << "unknown";
...
Bot::Bot() : pnj(std::make_unique<GameCharacter>()) {
...
bool Bot::champVisuel(GameCharacter *user) {
        ...
        if (bbopGetDistance(pnj->getPosition(), user->getPosition()) < range) {
                ...
                CollisionBox partie[5] = {pnj->getRightArm().getCollisionBox(),
                ...
                for (int i = 0; i < 21; i++) {
                        ...
                        Vector2f step = {(range * cos(theta[i])) / 5,
                                         (range * sin(theta[i])) / 5};
...
bool Bot::detectPoint(CollisionBox *menber, Vector2f point) {
        for (int i = 0; i < 5; i++) {
                if (point.x >= menber[i].getLeft() &&
                    point.x <= menber[i].getRight() &&
                    point.y >= menber[i].getTop() &&
                    point.y <= menber[i].getBottom()) {
                        return true;
                }
        }
        return false;
...
bool Bot::patrolZone() {
        for (int i = 0; i < 3; i++) {
                if (bbopGetDistance(pnj->getPosition(), Chokpoint[cpt]) <
                    500.f) {
                        return true;
                }
        }
...
bool Bot::moveToPoint(Vector2f point) {
        if (bbopGetDistance(point, pnj->getPosition()) > 10.f) {
                if (point.x - pnj->getPosition().x > 0.f) {
                        pnj->goRight();
                        pnj->lookAt(Vector2f(pnj->getPosition().x + 5,
                                             pnj->getPosition().y));
                } else {
                        pnj->goLeft();
                        pnj->lookAt(Vector2f(pnj->getPosition().x - 5,
...
void Bot::setChockPoint(Vector2f s1, Vector2f s2, Vector2f s3) {
        Chokpoint = new Vector2f[3]{s1, s2, s3};
}

Vector2f Bot::getSeekPosition() { return SeekPosition; }
...
void Bot::Draw(GLint *renderUniforms) const { pnj->Draw(renderUniforms); }

GameCharacter *Bot::getCharacter() { return pnj.get(); }

Bot::State Bot::getState() { return etat; }
